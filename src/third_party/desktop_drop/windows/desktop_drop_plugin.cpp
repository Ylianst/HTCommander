...
namespace {
    ...
    HRESULT DesktopDropTarget::Drop(IDataObject *pDataObj, DWORD grfKeyState, POINTL pt, DWORD *pdwEffect) {
        ...
        // Fallback: if the drop carried no files (CF_HDROP), try to read it as
        // text / a URL. This lets the app receive links dragged from a web
        // browser (delivered as CFSTR_INETURLW or CF_UNICODETEXT), not just
        // files. HTCommander patch — not present in upstream desktop_drop.
        if (list.empty()) {
            const UINT url_format = RegisterClipboardFormatW(L"UniformResourceLocatorW");
            UINT text_formats[2] = {url_format, CF_UNICODETEXT};
            for (UINT text_format : text_formats) {
                if (text_format == 0) {
                    continue;
                }
                FORMATETC text_fmt = {static_cast<CLIPFORMAT>(text_format), nullptr,
                                      DVASPECT_CONTENT, -1, TYMED_HGLOBAL};
                STGMEDIUM text_med;
                if (pDataObj->QueryGetData(&text_fmt) != S_OK) {
                    continue;
                }
                if (pDataObj->GetData(&text_fmt, &text_med) == S_OK) {
                    PVOID data = GlobalLock(text_med.hGlobal);
                    if (data != nullptr) {
                        std::wstring wide(reinterpret_cast<wchar_t *>(data));
                        std::string text = ws2s(wide);
                        if (!text.empty()) {
                            list.push_back(flutter::EncodableValue(text));
                        }
                        GlobalUnlock(text_med.hGlobal);
                    }
                    ReleaseStgMedium(&text_med);
                }
                if (!list.empty()) {
                    break;
                }
